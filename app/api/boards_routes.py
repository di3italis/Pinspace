...
def boards_add():
    ...
    board = Board(description=body["description"].strip(), ownerId=current_user.id)
...
def boards_addPin(id, pid):
    ...
    boardPin = BoardPin(pinId=pid, boardId=id)
...
def boards_pin(id):
    ...
    return jsonify({"board_pins": [boardPin.to_dict() for boardPin in board_pins]})
